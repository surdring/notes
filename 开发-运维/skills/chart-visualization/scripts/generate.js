...
async function main() {
  if (process.argv.length < 3) {
    ...
    console.error(
      'Windows PowerShell example: node ".\\scripts\\generate.js" \'{"tool":"generate_line_chart","args":{"data":[],"title":"..."}}\'',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Pass a valid JSON string or an existing UTF-8 JSON file path.");
    process.exit(1);
  }
  ...
  for (const item of specs) {
    ...
    if (!chartType) {
      ...
      console.error(`Available tools: ${Object.keys(CHART_TYPE_MAP).join(", ")}`);
      process.exitCode = 1;
      continue;
    }
