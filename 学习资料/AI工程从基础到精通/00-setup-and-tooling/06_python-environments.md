...
- 编写包含可选依赖组的 `pyproject.toml` 并生成 lockfile 以实现可复现性
- 诊断并修复常见陷阱：全局安装、pip/conda 混用、CUDA 版本不匹配
- 为存在冲突依赖的项目实施按阶段划分的环境策略

## 问题

你为微调项目安装了 PyTorch 2.4。下周，另一个项目需要 PyTorch 2.1，因为它的 CUDA 构建是固定的。你全局升级后，第一个项目崩溃了。你降级后，第二个项目崩溃了。

这就是依赖地狱。它在 AI/ML 工作中经常发生，因为：

- PyTorch、JAX 和 TensorFlow 各自携带自己的 CUDA 绑定
- 模型库固定了特定版本的框架
- 全局 `pip install` 会覆盖之前的安装
- CUDA 11.8 构建与 CUDA 12.x 驱动不兼容（反之亦然）

解决方案：每个项目获得自己隔离的环境和独立的包。
...
比 `uv` 慢，但任何安装了 Python 的地方都可以使用。

### 选项 3：conda（当需要时）

Conda 管理非 Python 依赖，如 CUDA 工具包、cuDNN 和 C 库。在以下情况下使用：

- 需要特定版本的 CUDA 工具包但不想系统级安装
- 在无法安装系统包的共享集群上
- 某个库的安装说明写着「使用 conda」
...
一条规则：如果你为环境使用 conda，那么该环境中所有包都使用 conda。在 conda 环境中混用 `pip install` 会导致难以调试的依赖冲突。

### 本课程：按阶段划分的策略

你可以为整个课程创建一个环境。不要这样做。不同阶段需要不同（有时是冲突的）依赖。
...
每个 Python 项目都应该有一个 `pyproject.toml`。它把 `setup.py`、`setup.cfg` 和 `requirements.txt` 合并到一个文件中。
...
Lockfile 将每个依赖（包括传递依赖）固定在确切的版本上。这保证了可复现性：任何人从 lockfile 安装都会得到完全相同的包。

```bash
# 使用 uv add 时，uv 自动生成 uv.lock
uv add numpy
...
如果必须在 conda 中使用 pip（某些包只有 pip 版本），先安装所有 conda 包，最后再装 pip 包。

### 3. 忘记激活

```bash
python train.py           # 使用系统 Python，包缺失
source .venv/bin/activate
...
虚拟环境大小 200MB-2GB。它们是本地的，不能在机器之间移植。改为提交 `pyproject.toml` 和 lockfile。
...
4. 故意全局安装一个包（不激活 venv），注意它的位置，然后卸载它

## 关键术语

| 术语 | 人们常说的 | 实际含义 |
|------|-----------|---------|
| 虚拟环境 | "一个 venv" | 包含 Python 解释器和包的隔离目录，与系统 Python 分离 |
| Lockfile | "固定依赖" | 列出每个包及其确切版本的文件，保证跨机器的相同安装 |
| pyproject.toml | "新的 setup.py" | 标准 Python 项目配置文件，取代 setup.py/setup.cfg/requirements.txt |
| 传递依赖 | "依赖的依赖" | 包 B 依赖 C；如果你安装 A 而 A 依赖 B，那么 C 是 A 的传递依赖 |
| CUDA 不匹配 | "我的 GPU 不工作" | PyTorch 编译时的 CUDA 版本与 GPU 驱动支持的版本不同 |